...
def build_fermentation_one_time_step(discretization: str='collocation',n_f_elements_t: int=1,total_f_elements_t:int=50,current_start_time_sconds: float=0,M0_prev_input: float=0,C0_prev_input: dict={'CS':0, 'XS':0, 'LS':0,'C':0,'G':0, 'X':0, 'F':0, 'E':0,'AC':0,'Cell':0,'Eth':0,'CO2':0,'ACT':0,'HMF':0,'Base':0}) -> pe.ConcreteModel():

    # ------------pyomo model------------------------------------------------
    m = pe.ConcreteModel(name='fermentation_model')
    # ------------shared scalars with hydrolisis model ----------------------
    m.final_time = pe.Param(initialize=(190*(60)*(60))/total_f_elements_t,doc='final simulation time with respect to 0 seconds [s]')  
...
if __name__ == '__main__':
    ...
    if v11=='ONE_TIME_STEP_SIMULATION':
        new_solver='knitro'
        solver_list=['knitro','conopt4','baron','ipopth']
        tee=False
        discretization_type_fer='collocation'
        finite_elem_t_fer=4 # 1 meaning that we will consider ony one finite element at a time
        total_elements=50 #total number of finite elements to be considered (not including 0)
        total_sim_time=190*(60)*(60) #Total batch time in seconds TODO: check that this agrees here and in model
        step=total_sim_time/total_elements
        ...
        # for disc_time in [0]:
            ...
            if disc_time!=0: # Update initial conditions as previous condition
                mad=build_fermentation_one_time_step(discretization=discretization_type_fer,n_f_elements_t=finite_elem_t_fer,total_f_elements_t=total_elements,current_start_time_sconds=current_start_time,M0_prev_input=M0_prev,C0_prev_input=C0_prev)  
                mad=initialize_model(mad,from_feasible=True,feasible_model='prev_init')          
            else: # Use default initial conditions
                mad=build_fermentation_one_time_step(discretization=discretization_type_fer,n_f_elements_t=finite_elem_t_fer,total_f_elements_t=total_elements,current_start_time_sconds=current_start_time) 
