...
def build_hydrolisis_convergence_test3(time: float=170*(3600),discretization: str='collocation',n_f_elements_x: int=10,n_f_elements_t: int=10) -> pe.ConcreteModel(): #TODO: MODIFY INPUTS
    ...
    def _liquid_mu_definition(m,t,x):
        return m.liquid_mu[t,x] == (A_W*pe.exp(B_W/m.T))          +       (m.C[t,x,'G']*m.rho_soluble*A_G*pe.exp(B_G/m.T))
    m.liquid_mu_definition=pe.Constraint(m.t,m.x, rule=_liquid_mu_definition)
    
    def _slurry_mu_definition(m,t,x): #NOTE: Here I am using article information that assumes that initial solid content is 25%
        a_0=1
        a_1=2.5
        a_2=10.5
        phi_initial=0.25 # Initial fraction of solid particles, considering conversion factors
        solid_fraction_entrance=(m.CFEED['CS']+m.CFEED['XS']+m.CFEED['LS'])/phi_initial
        return m.slurry_mu[t,x] == (a_0+a_1*((m.C[t,x,'CS']+ m.C[t,x,'XS']+m.C[t,x,'LS'])/solid_fraction_entrance)+a_2*(((m.C[t,x,'CS']+ m.C[t,x,'XS']+m.C[t,x,'LS'])/solid_fraction_entrance)**2))*m.liquid_mu[t,x]
    ...
    m.eta_pH=pe.Param(m.x,initialize=_eta_pH_init, doc='pH efficiency factor. Value between 0 and 1') #TODO: find a parameter from the plot in figure B.8 assuming constant pH, or improve the pH modeling. 
