...
#-------- this is required to apply dsda and ldbd (however when using variable continuous processing time these disjunctions now serve a purpose!!!!)----------------------------------------
    ...
    for I_J in m.I_J:
        ...
        m.YR2[I,J]=pe.BooleanVar(m.ordered_set2[I,J],initialize=False)
    ...
    #     def _build_YR2_Disjunct(m,indexN): #TODO: THINK ABOUT SOMETHING SIMILAR TO THIS TO DEFINE THE PART RELATED TO VARIABLE PROCESSING TIMES
    #         def _DEF_Nref(m):
    #             return m.model().Nref[I,J]==indexN
    #         m.DEF_Nref=pe.Constraint(rule=_DEF_Nref)
    #         # TODO: Do I have to use setattr here?
...
#-------- this is required to apply dsda and ldbd (however when using variable continuous processing time these disjunctions now serve a purpose!!!!)----------------------------------------
    ...
    m.obj = pe.Objective(rule=_obj, sense=pe.minimize)   
    return m
# Use this code to solve with GDP methods. TODO: I still have to work in the formulation to decrease the complexity of the GDP part of the problem.
def scheduling_and_control_gdp_N_solvegdp(x_initial: list=[4,4,5,5,3,3,3,2,2,3,3,2,2,2,3,2]):
    ...
    for I in m.I_reactions:
        for J in m.J_reactors:
            ...
            def _YRinit(m,ordered_set):
                if m.ordered_set[I,J]==(x_initial[positcui]+m.minTau[I,J]-1):
                    return True 
                else:
                    return False
...
#-------- this is required to apply dsda and ldbd (however when using variable continuous processing time these disjunctions now serve a purpose!!!!)----------------------------------------
    ...
    for I_J in m.I_J:
        ...
        def _YR2init(m,ordered_set2):
            if m.ordered_set2[I,J]== x_initial[positcui]-1:
                return True
            else:
                return False       
...
def scheduling_and_control_gdp_N_approx_sequential_naive(x_initial: list=[4,4,5,5,3,3,3,2,2,3,3,2,2,2,3,2],last_time_hours: float=14, demand_p1_kmol: float=1,demand_p2_kmol: float=1,sequential: bool=False,max_capacity: bool=False):
    # Data
    Infty=10 # TODO: BE CAREFULL, NUMERICAL ISSUES IF THIS HAS A VERY HIGH VALUE!!!!!!
    # ------------pyomo model------------------------------------------------
    #------------------------------------------------------------------------
...
#-------- this is required to apply dsda and ldbd (however when using variable continuous processing time these disjunctions now serve a purpose!!!!)----------------------------------------
    ...
    if max_capacity:
        def _obj(m):
            return -sum(sum(sum(m.X[I, J, T]*m.B[I,J,T]  for J in m.J_reactors)  for I in m.I_reactions)  for T in m.T)  
        ...
        def _obj(m):
            return sum(sum(sum(m.X[I, J, T]*m.varTime[I,J]  for J in m.J_reactors)  for I in m.I_reactions)  for T in m.T)  
            # return -sum(sum(sum(sum(m.CC[I,J,T][N]*m.X[I, J, T] for N in m.N[I,J,T] if N==m.N[I,J,T].last()) for J in m.J_dynamics) for I in m.I_dynamics) for T in m.T) 
        m.obj = pe.Objective(rule=_obj, sense=pe.minimize)  
    if sequential:
        def _obj_scheduling(m):
...
def scheduling_and_control_gdp_N_GBD(x_initial: list=[4,4,5,5,3,3,3,2,2,3,3,2,2,2,3,2],last_time_hours: float=14, demand_p1_kmol: float=1,demand_p2_kmol: float=1,sequential: bool=False,max_capacity: bool=False):
    # Data
    Infty=10 # TODO: BE CAREFULL, NUMERICAL ISSUES IF THIS HAS A VERY HIGH VALUE!!!!!!
    # ------------pyomo model------------------------------------------------
    #------------------------------------------------------------------------
    ...
    for I in m.I_reactions:
        for J in m.J_reactors:
            ...
            def _build_disjuncts(m,indexTau):  #Disjuncts for first Boolean variable
                ...
                # #----------- Variable processing times----------------------------------------------------------------
                def _DEF_VAR_TIME(m):
                    return m.model().varTime[I,J]==min([pe.value(m.model().tau_p[I,J]),m.maxTau[I,J]*m.delta])
                m.DEF_VAR_TIME=pe.Constraint(rule=_DEF_VAR_TIME,doc='Assignment of variable time value')
                # m.DEF_VAR_TIME.display()
...
def problem_logic_scheduling(m):
    ...
    return logic_expr
def problem_logic_scheduling_tau_only(m):
    logic_expr = []
    for I in m.I_reactions:
        for J in m.J_reactors:
            for index in m.ordered_set[I,J]:
    ...
    return logic_expr
def problem_logic_scheduling_dummy(m):
    logic_expr = []
