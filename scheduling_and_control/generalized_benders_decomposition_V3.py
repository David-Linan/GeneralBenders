...
# ###############################################################################
    ...
    # We have linking constraints, so we can change the domain of variables X to continuous in subproblems
    ...
    if sub.dsda_status=='Optimal':
        ...
        #TODO: rewrite!!
        mas.cuts.add(sum(sum( sub.dual[sub.const_link_VarTime[I,J]]*(mas.varTime[I,J]-pe.value(sub.link_varTime[I,J])) for I in mas.I_reactions)  for J in mas.J_reactors)+sum(sum(sum(  sub.dual[sub.const_link_B[I,J,T]]*( mas.B[I,J,T]-pe.value(sub.link_B[I,J,T])   )      for I in mas.I) for J in mas.J) for T in mas.T)+sum(sum(sum(  sub.dual[sub.const_link_X[I,J,T]]*( mas.X[I,J,T]-pe.value(sub.link_X[I,J,T])   )      for I in mas.I) for J in mas.J) for T in mas.T)<=0)
    ...
    #3: Solve primal problem
        ...
        if sub.dsda_status=='Optimal':
            ...
            if sub.UBD_new<sub.UBD:
                generate_initialization(m=sub,model_name='current_best_GBD_V3_subproblem_naive')
            sub.UBD=sub.UBD_new
            # if sub.UBD- mas.LBD <=epsilon:
            #     break
...
# END OF GENERALIZED BENDERS DECOMPOSITION---------------------------------------------------------------------------------------------------------------------
    ...
    sub=initialize_model(sub,from_feasible=True,feasible_model='current_best_GBD_V3_subproblem_naive')
